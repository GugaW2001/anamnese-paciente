    ...
    Tables: {
      ...
      patients_queue: {
        ...
        Relationships: [
          ...
          {
            foreignKeyName: 'patients_queue_send_list_id_fkey'
            columns: ['send_list_id']
            isOneToOne: false
    ...
    Functions: {
      ...
      claim_next_message: {
        ...
        Returns: {
          ...
          message_body: string
          patient_name: string
          phone_1_whatsapp_valid: boolean
          phone_2: string
          phone_2_whatsapp_valid: boolean
          phone_3: string
          phone_3_whatsapp_valid: boolean
          ...
          whatsapp_validated_format: string
        }[]
      }
      claim_next_message_for_instance: {
        Args: {
      ...
      enqueue_patient: {
        Args: {
          ...
          p_phone_1_whatsapp_valid?: boolean
          p_phone_2?: string
          ...
          p_send_after?: string
          p_send_list_id?: string
          p_status?: string
          p_time_proce?: string
          p_whatsapp_checked_at?: string
        }
        Returns: {
...
//   send_list_id: uuid (nullable)
//   phone_1_whatsapp_valid: boolean (nullable)
//   campaign_kind: text (nullable)
...
//   UNIQUE pacientes_registro_id_key: UNIQUE (registro_id)
// Table: patient_consent
...
// Table: patients_queue
//   CHECK patients_queue_campaign_kind_check: CHECK (((campaign_kind IS NULL) OR (campaign_kind = ANY (ARRAY['confirmation'::text, 'post_attendance'::text]))))
...
//   FOREIGN KEY patients_queue_send_list_id_fkey: FOREIGN KEY (send_list_id) REFERENCES send_lists(id) ON DELETE SET NULL
// Table: patients_queue_archive
//   PRIMARY KEY patients_queue_archive_pkey: PRIMARY KEY (id)
// Table: send_lists
//   PRIMARY KEY send_lists_pkey: PRIMARY KEY (id)
//   CHECK send_lists_status_check: CHECK ((status = ANY (ARRAY['draft'::text, 'queued'::text, 'in_progress'::text, 'completed'::text, 'cancelled'::text])))
// Table: system_config
...
//     WITH CHECK: true
// Table: send_lists
//   Policy "Allow all authenticated operations on send_lists" (ALL, PERMISSIVE) roles={authenticated}
//     USING: true
//     WITH CHECK: true
// Table: system_config
...
//    RETURNS TABLE(id uuid, patient_name text, phone_number text, message_body text, instance_id text, instance_name text, attempt_count integer, journey_id uuid, provider_message_id text, provider_chat_id text, locked_instance_id text, phone_attempt_index integer, phone_2 text, phone_3 text, last_phone_used text, phone_1_whatsapp_valid boolean, phone_2_whatsapp_valid boolean, phone_3_whatsapp_valid boolean, whatsapp_valid boolean, whatsapp_checked_at timestamp with time zone, whatsapp_validated_format text)
//    LANGUAGE plpgsql
//   AS $function$
//   DECLARE
//     v_instance_id TEXT;
...
//   BEGIN
//     SELECT wi.id::TEXT, wi.instance_name
...
//         AND (pq.locked_instance_id IS NULL OR pq.locked_instance_id = v_instance_id)
...
//       locked_instance_id = COALESCE(public.patients_queue.locked_instance_id, v_instance_id),
//       updated_at = NOW()
//     FROM next_msg
...
//       public.patients_queue.phone_1_whatsapp_valid,
//       public.patients_queue.phone_2_whatsapp_valid,
//       public.patients_queue.phone_3_whatsapp_valid,
...
//   $function$
//
// FUNCTION claim_next_message_for_instance(text, text, text, integer)
//   CREATE OR REPLACE FUNCTION public.claim_next_message_for_instance(p_worker_id text, p_instance_id text, p_instance_name text, p_max_attempts integer DEFAULT 3)
//    RETURNS TABLE(id uuid, patient_name text, phone_number text, message_body text, instance_id text, instance_name text, attempt_count integer, journey_id uuid, provider_message_id text, provider_chat_id text, locked_instance_id text, phone_attempt_index integer, phone_2 text, phone_3 text, last_phone_used text, phone_1_whatsapp_valid boolean, phone_2_whatsapp_valid boolean, phone_3_whatsapp_valid boolean, whatsapp_valid boolean, whatsapp_checked_at timestamp with time zone, whatsapp_validated_format text)
//    LANGUAGE plpgsql
//   AS $function$
//   BEGIN
...
//   $function$
//
// FUNCTION enqueue_patient(text, text, text, text, boolean, timestamp with time zone, text, integer, text, uuid, text, text, text, text, text, text, text, text, text, text, text, integer, text, boolean, boolean, boolean, timestamp with time zone, uuid, text)
//   CREATE OR REPLACE FUNCTION public.enqueue_patient(p_patient_name text, p_phone_number text, p_message_body text, p_status text DEFAULT 'queued'::text, p_is_approved boolean DEFAULT true, p_send_after timestamp with time zone DEFAULT now(), p_notes text DEFAULT NULL::text, p_attempt_count integer DEFAULT 0, p_dedupe_kind text DEFAULT 'original'::text, p_origin_queue_id uuid DEFAULT NULL::uuid, p_canonical_phone text DEFAULT NULL::text, p_dedupe_hash text DEFAULT NULL::text, p_data_nascimento text DEFAULT NULL::text, p_data_exame text DEFAULT NULL::text, p_procedimentos text DEFAULT NULL::text, p_horario_inicio text DEFAULT NULL::text, p_horario_final text DEFAULT NULL::text, p_time_proce text DEFAULT NULL::text, p_phone_2 text DEFAULT NULL::text, p_phone_3 text DEFAULT NULL::text, p_locked_instance_id text DEFAULT NULL::text, p_phone_attempt_index integer DEFAULT 1, p_last_phone_used text DEFAULT NULL::text, p_phone_1_whatsapp_valid boolean DEFAULT NULL::boolean, p_phone_2_whatsapp_valid boolean DEFAULT NULL::boolean, p_phone_3_whatsapp_valid boolean DEFAULT NULL::boolean, p_whatsapp_checked_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_send_list_id uuid DEFAULT NULL::uuid, p_campaign_kind text DEFAULT NULL::text)
...
//     v_dedupe_hash TEXT;
//     v_new_id UUID;
//     v_campaign_kinds TEXT[] := ARRAY['retry_phone2', 'retry_phone3', 'followup_confirm', 'confirmation', 'post_attendance'];
//   BEGIN
...
//     END IF;
//
//     IF p_dedupe_kind = ANY(v_campaign_kinds) AND p_origin_queue_id IS NOT NULL THEN
//       IF EXISTS (
//         SELECT 1
//         FROM public.patients_queue pq
...
//           AND pq.status IN ('queued', 'sending', 'delivered')
//       ) THEN
//         RETURN QUERY SELECT NULL::UUID, 'duplicate_recent'::TEXT, 'Mensagem de campanha duplicada para esta origem'::TEXT;
//         RETURN;
//       END IF;
//     END IF;
//
//     IF p_dedupe_kind != ALL(v_campaign_kinds) THEN
//       IF EXISTS (
//         SELECT 1
//         FROM public.patients_queue pq
//         WHERE pq.canonical_phone = v_canonical_phone
...
//       p_data_exame,
//       p_procedimentos,
//       CASE WHEN p_horario_inicio IS NULL OR p_horario_inicio = '' THEN NULL ELSE p_horario_inicio::time END,
//       CASE WHEN p_horario_final IS NULL OR p_horario_final = '' THEN NULL ELSE p_horario_final::time END,
//       p_time_proce,
//       p_phone_2,
//       p_phone_3,
//       CASE WHEN p_locked_instance_id IS NULL OR p_locked_instance_id = '' THEN NULL ELSE p_locked_instance_id::uuid END,
//       p_phone_attempt_index,
//       p_last_phone_used,
//       p_phone_1_whatsapp_valid,
//       p_phone_2_whatsapp_valid,
//       p_phone_3_whatsapp_valid,
//       p_whatsapp_checked_at,
//       p_send_list_id,
//       p_campaign_kind,
//       COALESCE(
...
//   $function$
//
// FUNCTION enqueue_patient_v2(text, text, text, text, boolean, timestamp with time zone, text, integer, text, uuid, text, text, text, text, text, text, text, text, text, text, text, integer, text, boolean, boolean, boolean, timestamp with time zone)
//   CREATE OR REPLACE FUNCTION public.enqueue_patient_v2(p_patient_name text, p_phone_number text, p_message_body text, p_status text DEFAULT 'queued'::text, p_is_approved boolean DEFAULT true, p_send_after timestamp with time zone DEFAULT now(), p_notes text DEFAULT NULL::text, p_attempt_count integer DEFAULT 0, p_dedupe_kind text DEFAULT 'original'::text, p_origin_queue_id uuid DEFAULT NULL::uuid, p_canonical_phone text DEFAULT NULL::text, p_dedupe_hash text DEFAULT NULL::text, p_data_nascimento text DEFAULT NULL::text, p_data_exame text DEFAULT NULL::text, p_procedimentos text DEFAULT NULL::text, p_horario_inicio text DEFAULT NULL::text, p_horario_final text DEFAULT NULL::text, p_time_proce text DEFAULT NULL::text, p_phone_2 text DEFAULT NULL::text, p_phone_3 text DEFAULT NULL::text, p_locked_instance_id text DEFAULT NULL::text, p_phone_attempt_index integer DEFAULT 1, p_last_phone_used text DEFAULT NULL::text, p_phone_1_whatsapp_valid boolean DEFAULT NULL::boolean, p_phone_2_whatsapp_valid boolean DEFAULT NULL::boolean, p_phone_3_whatsapp_valid boolean DEFAULT NULL::boolean, p_whatsapp_checked_at timestamp with time zone DEFAULT NULL::timestamp with time zone)
//    RETURNS TABLE(id uuid, status text, error_message text)
//    LANGUAGE plpgsql
//   AS $function$
//   DECLARE
...
//     v_canonical_phone text;
//     v_dedupe_hash text;
//     v_locked_instance_uuid uuid;
//     v_horario_inicio time;
//     v_horario_final time;
//   BEGIN
//     -- A. Normalização de Inputs (Tratar strings vazias como NULL)
//     v_canonical_phone := COALESCE(NULLIF(p_canonical_phone, ''), regexp_replace(p_phone_number, '\D', '', 'g'));
//     v_dedupe_hash := COALESCE(NULLIF(p_dedupe_hash, ''), md5(v_canonical_phone || '|' || COALESCE(p_data_exame, '') || '|' || p_message_body));
//
//     -- Conversão segura para UUID
//     IF p_locked_instance_id IS NOT NULL AND p_locked_instance_id <> '' THEN
//       BEGIN
//         v_locked_instance_uuid := p_locked_instance_id::uuid;
//       EXCEPTION WHEN OTHERS THEN
//         v_locked_instance_uuid := NULL;
//       END;
//     END IF;
//
//     -- B. Conversão segura para TIME (Tratar strings vazias ou inválidas como NULL)
//     BEGIN
//       v_horario_inicio := NULLIF(p_horario_inicio, '')::time;
//     EXCEPTION WHEN OTHERS THEN
//       v_horario_inicio := NULL;
//     END;
//
//     BEGIN
//       v_horario_final := NULLIF(p_horario_final, '')::time;
//     EXCEPTION WHEN OTHERS THEN
//       v_horario_final := NULL;
//     END;
//
//     -- C. Inserção
//     INSERT INTO public.patients_queue (
...
//       p_phone_2,
//       p_phone_3,
//       v_locked_instance_uuid,
//       COALESCE(p_phone_attempt_index, 1),
//       p_last_phone_used,
//       p_phone_1_whatsapp_valid,
//       p_phone_2_whatsapp_valid,
//       p_phone_3_whatsapp_valid,
...
//       now()
//     )
//     RETURNING public.patients_queue.id INTO v_new_id;
//
//     RETURN QUERY SELECT v_new_id, 'success'::text, NULL::text;
//
//   EXCEPTION
//     WHEN unique_violation THEN
//       RETURN QUERY SELECT NULL::uuid, 'duplicate_recent'::text, 'Recent message already queued for this phone/exam combination.'::text;
//     WHEN OTHERS THEN
//       RETURN QUERY SELECT NULL::uuid, 'error'::text, SQLERRM;
//   END;
//   $function$
//
...
// Table: pacientes
//   CREATE UNIQUE INDEX pacientes_registro_id_key ON public.pacientes USING btree (registro_id)
// Table: patient_consent
...
// Table: patients_queue
//   CREATE INDEX idx_patients_queue_campaign_kind ON public.patients_queue USING btree (campaign_kind)
...
//   CREATE UNIQUE INDEX idx_patients_queue_dedupe_hash_queued ON public.patients_queue USING btree (dedupe_hash) WHERE (status = 'queued'::queue_status)
...
//   CREATE INDEX idx_patients_queue_instance_claim ON public.patients_queue USING btree (locked_instance_id, queue_order, send_after) WHERE ((status = 'queued'::queue_status) AND (is_approved = true) AND (locked_by IS NULL))
...
//   CREATE INDEX idx_patients_queue_needs_validation ON public.patients_queue USING btree (whatsapp_checked_at) WHERE ((status = 'queued'::queue_status) AND (whatsapp_checked_at IS NULL))
...
//   CREATE INDEX idx_patients_queue_send_list_id ON public.patients_queue USING btree (send_list_id)
...
// Table: send_lists
//   CREATE INDEX idx_send_lists_exam_date ON public.send_lists USING btree (exam_date)
//   CREATE INDEX idx_send_lists_locked_instance_id ON public.send_lists USING btree (locked_instance_id)
//   CREATE INDEX idx_send_lists_status ON public.send_lists USING btree (status)
// Table: webhook_events_raw

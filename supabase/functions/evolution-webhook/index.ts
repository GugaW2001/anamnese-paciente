...
function detectEventType(payload: any): EventType | null {
  ...
  if (t.includes('messages.update')) {
    ...
    if (status.includes('read') || status.includes('read_ack') || status.includes('seen'))
      return 'read'
  ...
  if (t.includes('send.message') || (t.includes('send') && t.includes('message')))
    ...
    for (const p of phonesToTry) {
      const queueMessage =
        ...
        (await restSelectOne(
          ...
          [
            ['phone_number', `eq.${p}`],
            ['status', 'in.(queued,sending,delivered)'],
          ],
          'created_at.desc',
        )) ||
        (await restSelectOne(
...
async function resolveMessageIdByProvider(
  providerMessageId: string | null,
): Promise<string | null> {
  if (!providerMessageId) return null
  const row = await restSelectOne('patients_queue', 'id', [
    ['provider_message_id', `eq.${providerMessageId}`],
  ])
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (eventType === 'send_accepted') {
      ...
      // Isso e suficiente para confirmar que o envio foi concluido pelo nosso sistema.
      ...
      if (resolvedMessageId) {
        ...
        console.log(
          `[evolution-webhook] send_accepted: patients_queue ${resolvedMessageId} marcado como delivered.`,
        )
      }
    } else if (eventType === 'delivered') {
      if (resolvedMessageId) {
      ...
      if (journeyId) {
        try {
          ...
          if (normalizedPhone && providerMessageId) {
            const existingInbound = await restSelectOne('journey_messages', 'id', [
              ['provider_message_id', `eq.${providerMessageId}`],
            ])
